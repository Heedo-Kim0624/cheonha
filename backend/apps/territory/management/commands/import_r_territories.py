...
class Command(BaseCommand):
    help = "Import R territory polygons from the bundled GPKG-derived fixture."

    def add_arguments(self, parser):
        parser.add_argument(
            "--fixture",
            default="",
            help="Optional fixture JSON path. Defaults to territory/fixtures/r_territories.json.",
        )
        parser.add_argument(
            "--company-app",
            default=DEFAULT_COMPANY_APP,
            help="Company app code for the R team. Defaults to cheonha.",
        )
        parser.add_argument(
            "--replace",
            action="store_true",
            help="Delete existing R territories for the company that are not in the fixture.",
        )

    def handle(self, *args, **options):
        fixture = options.get("fixture")
        fixture_path = Path(fixture) if fixture else Path(__file__).resolve().parents[2] / "fixtures" / "r_territories.json"
            ...
            for item in rows:
                ...
                territory, was_created = Territory.objects.update_or_create(
                    ...
                    defaults={
                        ...
                        "color": item.get("color") or "#EC4899",
                        "note": "R GPKG imported territory",
                    },
                )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Imported R territories: created={created}, updated={updated}, deleted={deleted}, company_app={company_app}"
            )
        )
