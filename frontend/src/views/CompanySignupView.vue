<template>
  <div class="min-h-screen bg-slate-50 px-6 py-10 text-slate-900">
    <main class="mx-auto max-w-2xl rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      <div class="mb-6">
        <p class="text-sm font-bold text-lime-600">CLEVER 회사 계정 신청</p>
        <h1 class="mt-1 text-2xl font-bold">{{ inviteCompanyName }}</h1>
        <p class="mt-2 text-sm text-slate-500">
          회사 대표 계정은 CLEVER 관리자의 승인 후 사용할 수 있습니다.
        </p>
      </div>
      ...
      <form v-else class="space-y-5" @submit.prevent="submit">
        <label class="block">
          <span class="mb-2 block text-sm font-semibold text-slate-700">회사명</span>
          <input v-model.trim="form.company_name" required class="form-input" />
        </label>
        ...
        <RouterLink
          v-if="submitted"
          :to="loginPath"
          class="block w-full rounded-lg border border-slate-900 bg-slate-900 px-4 py-3 text-center font-bold text-white hover:bg-slate-800"
        >
          로그인 화면으로 이동
        </RouterLink>

        <button type="submit" class="w-full rounded-lg bg-lime-500 px-4 py-3 font-bold text-slate-900 hover:bg-lime-400 disabled:opacity-50" :disabled="saving || submitted">
          {{ saving ? '신청 중...' : submitted ? '신청 완료' : '회사 계정 신청' }}
        </button>
      </form>
    </main>
  </div>
...
import { fetchCompanySignup, submitCompanySignup } from '@/api/companyAdmin'
import { getCompanyApp } from '@/utils/companyApp'
...
const inviteCompanyName = computed(() => company.value?.name || '회사 초대')
const loginPath = computed(() => getCompanyApp(company.value?.code || 'cheonha').loginPath)
...
async function loadInvite() {
  ...
  try {
    const response = await fetchCompanySignup(token.value)
    company.value = response.data.company
    agreements.value = response.data.agreements || []
    form.company_name = company.value?.name || ''
  } catch (error) {
    notice.value = { type: 'error', message: error.response?.data?.detail || '초대 정보를 불러오지 못했습니다.' }
  } finally {
    loading.value = false
  }
}
...
async function submit() {
  ...
  notice.value = null
  try {
    const response = await submitCompanySignup(token.value, { ...form })
    company.value = response.data.company || company.value
    submitted.value = true
    notice.value = { type: 'success', message: response.data.detail || '신청이 완료되었습니다.' }
  } catch (error) {
    notice.value = {
      type: 'error',
      message:
